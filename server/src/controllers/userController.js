...
exports.setDefaultAddress = async (req, res) => {
    try {
        ...
        res.status(200).json(updatedUser.defaultAddress);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: error.message });
    }
...
exports.setDefaultPaymentMethod = async (req, res) => {
    try {
        ...
        if (!paymentMethod) {
            return res.status(404).json({ message: "Payment method not found or doesn't belong to user." });
        }
        ...
        res.status(200).json(updatedUser.defaultPaymentMethod);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: error.message });
    }
...
exports.removeDefaultAddress = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Default address removed successfully." });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: error.message });
    }
...
exports.removeDefaultPaymentMethod = async (req, res) => {
    try {
        const userID = parseInt(req.user.id);
        ...
        res.status(200).json({ message: "Default payment method removed successfully." });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: error.message });
    }
